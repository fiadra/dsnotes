...
\begin{proof}[Proof Chernoff's theorem]
...
\begin{theorem}[Egorov]
  ...
  < \delta \) and \( X_n \) converges to \( X \) uniformly on \( A_{\delta} \).
\end{theorem}
...
\begin{remark}
  ...
  \mathbb{R}^d \) are concentrated around a sphere with radius \( \sqrt{d} \).
\end{remark}
...
\begin{theorem}[Johnson-Lindenstrauss Lemma]
  ...
  \[
    ...
    (1+\varepsilon) \sqrt{k} \left\|v_{i} - v_{j}\right\| \right\} \geq 1- 3/2n.
  \]
\end{theorem}
...
\begin{definition}[Singular Value Decomposition]
  ...
  d}(\mathbb{R}) \), where we think of rows of \( A \) as points in a
  \( d \)-dimensional space, is the factorization
  \[
    A = UDV^{T},
  \]
  where the columns of \( U, V \) are orthonormal and the matrix \( D
...
\begin{remark}
  The decomposition above can be rewritten as
  \[
    A = \sum_{i} d_{ii}u_{i}v_{i}^{T}.
  \]
  Here each \( u_{i}v_{i}^{T} \) is a \( n \times d \) matrix.
\end{remark}
...
\begin{lemma}
  If \( A = UDV^{T} \) is the SVD, then
  \[
    Av_{i} = d_{ii} u_{i}, \; A^{T}u_{i} = d_{ii} v_{i}.
  \]
\end{lemma}
...
\begin{definition}[Singular values]
  If \( A = UDV^{T} \), then the \( i \)-th singular value is \(
...
\begin{remark}
  Notice that \( \sigma_{i}(A) u_{i}v_{i}^{T} \) is a \( 1
  \)-dimensional matrix whose rows are the \( v_{i} \)-components of \( A \).
\end{remark}
...
\begin{lemma}
  \[
    \left\|A - A_{k}\right\|_{F}^{2} = \sigma_{k+1}^{2}.
  \]
\end{lemma}

\begin{fact}
  The centroid of a set of data points must pass through the best-fitting line.
\end{fact}
